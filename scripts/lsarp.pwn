...
{
	SendMessageToAdmins(true, COLOR_ERROR, "[ADMIN-ALERT]: %s (%d) � sospetto di hack. (%s)", Character_GetOOCName(playerid), playerid, AC_Name[code]);
	return 1;
}
*/
...
hook OnPlayerDisconnect(playerid, reason)
{
	...
	string = str_format("%s � uscito dal server. [%s]", name, reasonName[reason]);
...
hook OnRconLoginAttempt(ip[], password[], success )
{
	...
	foreach(new i : Player)
	{
		...
		if(!strcmp(ip, ip2, true))
		{
			if(success)
			{
				SendMessageToAdmins(true, COLOR_ADMIN, "%s (%d) � entrato in RCON.", Character_GetOOCName(i), i);
			}
			else
			{

...
static LastSlowModeCommandTicks[MAX_PLAYERS] = {0, ...};
...
public OnPlayerCommandReceived(playerid, cmd[], params[], flags)
{
    ...
    if(!isAllowed)
    {
        SendMessageToAdmins(true, COLOR_ORANGE, "AdmAlert: %s (%d) ha utilizzato un comando proibito (/%s)", Character_GetOOCName(playerid), playerid, cmd);
	...
	if(flags & CMD_SLOW_MODE)
	{
		if(GetTickCount() - LastSlowModeCommandTicks[playerid] < 2000)
		{
			SendClientMessage(playerid, COLOR_ERROR, "Devi aspettare alcuni secondi prima di poter utilizzare questo comando.");
			return 0;
		}
	}
...
public OnPlayerCommandPerformed(playerid, cmd[], params[], result, flags)
{
	...
	if(flags & CMD_SLOW_MODE)
	{
		LastSlowModeCommandTicks[playerid] = GetTickCount();
	}
	return 1;
}
